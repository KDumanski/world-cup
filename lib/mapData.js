...
// ============================ CONTINENT-WIDE ============================
// Flatten every spot across all 16 host cities into one list — for the
// map-first view that starts zoomed out and clusters everything.

export function allSpots(cities) {
  const out = [];
  for (const city of cities) {
    for (const s of citySpots(city)) {
      out.push({ ...s, cityName: city.name, citySlug: city.slug, country: city.country });
    }
  }
  return out;
}

// All distinct nations across every city, with flag + total count.
export function allNations(cities) {
  const map = new Map();
  for (const city of cities) {
    for (const s of city.spots || []) {
      if (!map.has(s.nation)) map.set(s.nation, { nation: s.nation, flag: s.flag, count: 0 });
...
// All distinct types across every city, with color + total count.
export function allTypes(cities) {
  const map = new Map();
  for (const s of allSpots(cities)) {
    if (!map.has(s.type)) map.set(s.type, { type: s.type, color: s.color, count: 0, keywords: s.keywords });
    map.get(s.type).count++;
  }
  return [...map.values()].sort((a, b) => b.count - a.count);
}
